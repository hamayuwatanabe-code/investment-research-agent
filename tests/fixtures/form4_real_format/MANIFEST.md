...
| `officer_title_inconsistent.xml` | A reporting owner with `isOfficer=0` yet a genuinely non-empty `officerTitle` | Mirrors a real Phase 3E.4 Mac Live Smoke finding: `relationship_fields_inconsistent=True`, both raw fields kept exactly as reported, never "corrected" (Phase 3E.4); not added to the shared `_SCENARIOS` |
